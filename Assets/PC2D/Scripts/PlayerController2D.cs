...
public class PlayerController2D : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (ETCInput.GetButtonDown(PC2D.Input.JUMP))
        {
            _motor.Jump();
            _motor.DisableRestrictedArea();
        }

        _motor.jumpingHeld = ETCInput.GetButton(PC2D.Input.JUMP);

        // XY freedom movement
        if (_motor.motorState == PlatformerMotor2D.MotorState.FreedomState)
        {
            _motor.normalizedXMovement = ETCInput.GetAxis(PC2D.Input.HORIZONTAL);
            _motor.normalizedYMovement = ETCInput.GetAxis(PC2D.Input.VERTICAL);

            return; // do nothing more
        }

        // X axis movement
        if (Mathf.Abs(ETCInput.GetAxis(PC2D.Input.HORIZONTAL)) > PC2D.Globals.INPUT_THRESHOLD)
        {
            _motor.normalizedXMovement = ETCInput.GetAxis(PC2D.Input.HORIZONTAL);
        }
        else
        ...
        if (ETCInput.GetAxis(PC2D.Input.VERTICAL) != 0)
        {
            bool up_pressed = ETCInput.GetAxis(PC2D.Input.VERTICAL) > 0;
        ...
        else if (ETCInput.GetAxis(PC2D.Input.VERTICAL) < -PC2D.Globals.FAST_FALL_THRESHOLD)
        {
            _motor.fallFast = false;
        }

		if (ETCInput.GetButtonDown(PC2D.Input.DASH))
        {
            _motor.Dash();
        }
    }
